...
// `.trim()` em todos: espaço acidental no segredo gera 401/404 sem pista.
export const JIRA_BASE = (
  process.env["JIRA_BASE_URL"]?.trim() || "https://way2agile.atlassian.net"
).replace(/\/$/, "");
export const SP_FIELD = process.env["JIRA_SP_FIELD"]?.trim() || "customfield_10005";
export const SPRINT_FLD = process.env["JIRA_SPRINT_FIELD"]?.trim() || "customfield_10007";
export const REVIEWER_FIELD = process.env["JIRA_REVIEWER_FIELD"]?.trim() || "customfield_10200";
export const CHAMADOS_FIELD = process.env["JIRA_CHAMADOS_FIELD"]?.trim() || "customfield_12449";
